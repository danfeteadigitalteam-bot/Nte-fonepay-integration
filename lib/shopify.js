...
/**
 * Fetch the order total (shop currency) for a numeric Shopify order ID.
...
async function getOrderTotal(orderId) {
  ...
  const data = await adminGraphql(query, { id: `gid://shopify/Order/${numericId}` });
  const money = data?.order?.currentTotalPriceSet?.shopMoney;
  const amountRaw = money?.amount;
  const currencyCode = money?.currencyCode;

  const amount = Number(amountRaw);
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error('Shopify returned invalid order total');
  }
